...
void verificarMemoria(void *ptr, int llamado){
    if(!ptr){
        std::cout<<"Error occured in the "<<llamado<<" call.\n";
        exit(EXIT_FAILURE);
    }
}
...
int productoEscalar(vectores Vector1, vectores Vector2){
    int producto=0;
    producto=Vector1.getComponenteX()*Vector2.getComponenteX()+Vector1.getComponenteY()*Vector2.getComponenteY();
    return producto;
}
